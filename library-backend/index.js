...
const resolvers = {
  Query: {
      ...
      allBooks: (root, args) => {
        ...
        if(author && genre){
          return books.filter(book => book.author === author && book.genres.find(currGenre => currGenre === genre));
        }
        else if(author){
            ...
            return books.filter(book => book.genres.find(genre => genre === genreName));
        }
          return books
      },
      allAuthors:()=> {
          ...
          authors.forEach(author => {              
              ...
              authorInfo.push({name: author.name, born: author.born, bookCount: authorBooks.length})
          })
        return authorInfo;
      }
  },
  Mutation: {
    addBook: (root, args) => {
      ...
      if(!authors.find(author => author.name === newBook.author)){
        authors = authors.concat({name: newBook.author, born: null, id:uuid()})
      }
      return newBook;
    },
    addAuthor:(root, args) => {
